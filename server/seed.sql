...
('나는 누구인가 스스로 물으라 자신의 속얼굴이 드러나 보일 때까지 묻고 묻고 물어야 한다건성으로 묻지말고 목소리 속의 목소리로 귀 속의 귀에 대고 간절하게 물어야 한다해답은 그 물음 속에 있다','법정스님'),
...
('신이 할수 있다고 믿든 할수 없다고 믿든 믿는 대로 될것이다','헨리 포드'),
 ...
 *    mysql -u root < server/seed.sql -p -Dproject (데이터베이스 이름 project)
